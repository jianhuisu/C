...
//　　最优的情况下空间复杂度为:O(logn)；每一次都平分数组的情况 最差的情况下空间复杂度为：O( n )；

// 快排的时间复杂度分析
// 时间复杂度 平均 O(nlogn) 最坏 O(n^2)  初始序列 局部有序或者整体有序时 退化为冒泡排序 : 因为 中间枢纽一般选择第一个记录，为了改善这种情况 可以选择中间元素作为枢纽 来改善这种情况

// ！！！快速排序的平均性能优于 很多排序方法  所以要重点掌握.