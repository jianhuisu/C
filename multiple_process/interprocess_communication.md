...
`myPipe`就是这个管道的名称，基于Linux一切皆文件的理念，所以管道也是以文件的方式存在，我们可以用`ls`看一下，这个文件的类型是`p`，也就是 `pipe（管道）` 的意思：
...
这样就做到了两个进程各有两个`「 fd[0] 与 fd[1]」`，两个进程就可以通过各自的 `fd` 写入和读取同一个管道文件实现跨进程通信了。
...
在 shell 里面执行` A | B`命令的时候，`A` 进程和 `B` 进程都是 shell 创建出来的子进程，
A 和 B 之间不存在父子关系，它俩的父进程都是 shell。

所以说，在 shell 里通过`「|」`匿名管道将多个命令连接在一起，实际上也就是创建了多个子进程，那么在我们编写 `shell` 脚本时，能使用一个管道搞定的事情，就不要多用一个管道，这样可以减少创建子进程的系统开销。

所以，**对于匿名管道，它的通信范围是存在父子关系的进程**。因为管道没有实体，也就是没有管道文件，只能通过 `fork`来复制父进程 fd 文件描述符，来达到通信的目的。
...
消息队列不适合比较大数据的传输，因为在内核中每个消息体都有一个最大长度的限制，同时所有队列所包含的全部消息体的总长度也是有上限。在 Linux 内核中，会有两个宏定义`MSGMAX`和`MSGMNB`，它们以字节为单位.
...
Tips:
  ...
  - 共享内存很好的解决了消息队列通信过程中，存在用户态与内核态之间的数据拷贝开销
...
**信号量其实是一个整型的计数器，主要用于实现进程间的互斥与同步，而不是像管道一样,直接用缓存进程间通信的数据。**
...
如果进程在后台运行，可以通过`kil` 命令的方式给进程发送信号，但前提需要知道运行中的进程`PID`号，例如：
...
针对 TCP 协议通信的 socket 编程模型
 ...
 1. 服务端调用 bind，将绑定在 IP 地址和端口;
 1. 服务端调用 listen，进行监听；
 1. 服务端调用 accept，等待客户端连接；
 1. 客户端调用 connect，向服务器端的地址和端口发起连接请求；
 1. 服务端 accept 返回用于传输的 socket 的文件描述符；
...
(socket implement should reference some physics knowledge, not only software implements)
...
对于 UDP 来说，不需要要维护连接，那么也就没有所谓的发送方和接收方，甚至都不存在客端和服务端的概念，只要有一个 socket 多台机器就可以任意通信，因此每一个 UDP 的 socket 都需要 bind。
...
以上，就是进程间通信的主要机制了。你可能会问了，那线程间通信的方式呢？

#### 线程间通信

同个进程下的线程之间都是共享进程的资源，只要是共享变量都可以做到线程间通信，比如全局变量，所以对于线程间关注的不是通信方式，
而是关注多线程竞争共享资源的问题，信号量也同样可以在线程间实现互斥与同步：
