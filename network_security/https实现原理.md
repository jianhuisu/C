...
首先我们必须明确一点,默认情况下加密算法为`黑客`与`服务端/客户端`共同知晓,只是在密钥的知晓权上不同.
...
如果在商议的过程中，黑客介入进行截取，只要黑客获取到这个key，那么后面的对称加密就完全失效了。
...
因为加密和解密使用的是两个不同的密钥，所以这种算法叫作`非对称加密算法`。
 ...
 - 加密密钥PK(public ke钥)是向公众公开的,并且所有的客户端接收到服务器颁发的公钥都是相同的.
 - 解密的密钥(private key即私钥或者密钥),需要保密.
 - 加密算法E(encryption),解密算法D(decrypt)也都是公开的.

鉴于非对称加密的机制，我们可能会有这种思路：

 1. TCP三次握手成功以后,服务器先把公钥直接明文传输给浏览器
 1. 浏览器向服务器传数据前使用公钥加密,只有服务器拥有私钥才可以进行解密.
 1. 浏览器要接收服务器发送给它的数据(**服务器使用私钥加密后的数据**),浏览器可以用建立链接时get的公钥进行解密.
 
但是这个流程存在一个问题,如果公钥在传输过程中被劫持,存在以下安全隐患.

 1. 劫持者首先劫持到公钥,然后劫持者可以截取浏览器向服务器发送的数据,
 1. 劫持者持有公钥,但是不能使用公钥解析出浏览器中发送的原始数据.从浏览器客户端 到 服务器方向 的信道是安全的.
 1. 当服务端向客户端返回响应时,劫持者可以以用公钥解密得到来自服务器的加密数据,从而获得一些敏感信息,如用户个人中心的隐私数据等等.

综上所述:**通过一组公钥私钥,可以保证单个方向传输的安全性**.

很简单的一个日常示例:

局域网中调式app接口时,常用charles进行抓包.也就是手机的wifi中设置使用charles作为代理.
所有网络请求都要经过charles进行转发.手机中的所有网络活动可以说是被charles一览无余.
利用charles我们就可以验证刚才的过程.
...
看起来很严谨的样子,但是这种方案仍然存有一个隐患,`非对称加密解密算法计算量较大导致性能比较低,例如当需要传输大文件时,会严重拖慢性能`,所以https的加密没有使用这种方案.
...
世界上没有任何东西是完美的.上边的`对称加密 + 非对称加密`组合并非完美.
...
这样在双方都不会发现异常的情况下，中间人得到了浏览器生成的要用来对称加密算法的`密钥B`.
这就像放在冰箱中的雪糕都被人偷偷舔了一口.

!!!!注意!!!!:这种现象发生的根本原因是:**浏览器无法确认自己收到的公钥是来自官方还是来自第三方**.
...
整个响应信息主要分为两大部分`明文+签名`.用同样的算法对同样的明文加密应该得到同样签名.

(将密钥发放通道与数据发送通道隔离开.一个物理通道操作系统预置根证书)
